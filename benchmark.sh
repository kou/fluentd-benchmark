...
BENCHMARK_NAME="$1"
if [ -f "${BENCHMARK_NAME}.yml" ]; then
  DOCKER_COMPOSE_YML="${BENCHMARK_NAME}.yml"
elif [ -f "${BENCHMARK_NAME}/docker-compose.yml" ]; then
...
if [ -z "${DOCKER_COMPOSE_YML}" ] ; then
  ...
  (
    ls *.yml | $SED -e 's/\.yml$//'
    ls */docker-compose.yml | $SED -e 's,/docker-compose.yml$,,'
  ) | sort | uniq
  exit 1
fi
...
grep out_flowcounter_simple "${BENCHMARK_NAME}.out" | \
  ...
  tail -n +2 | \
  $HEAD -n -2 | \
  perl -I ~/work/perl/st/lib ~/work/perl/st/script/st --transpose-output --n --min --max --mean --median
